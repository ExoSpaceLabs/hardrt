...
static void policy_switcher(void* arg){ (void)arg; for(;;){
        if (!g_switch_done){
...
static void rr_counter_A(void* arg){ (void)arg; for(;;){ ++g_rrA; hrt_yield(); } }
static void rr_counter_B(void* arg){ (void)arg; for(;;){ ++g_rrB; hrt_yield(); } }

static void stopper_after_ticks(void* arg){ uint32_t ms = (uint32_t)(uintptr_t)arg; for(;;){ hrt_sleep(ms); hrt__test_stop_scheduler(); hrt_yield(); } }

static void test_policy_switch_rr_to_priority(void){
    hrt__test_reset_scheduler_state(); g_switch_done=0; g_rrA=g_rrB=0;
...
static void old_rr_task(void* arg){ (void)arg; for(;;){ ++g_old_rr; hrt_yield(); } }
static void new_rr_task(void* arg){ (void)arg; for(;;){ ++g_new_rr; hrt_yield(); } }

static void test_default_slice_change_affects_new_tasks_only(void){
    hrt__test_reset_scheduler_state(); g_new_rr=g_old_rr=0;
    ...
    a.timeslice = 2; int to = hrt_create_task(old_rr_task, NULL, so, 1024, &a);
...
const test_case_t* get_tests_runtime_tuning(int* out_count){ if(out_count)*out_count=(int)(sizeof(CASES)/sizeof(CASES[0])); return CASES; }
