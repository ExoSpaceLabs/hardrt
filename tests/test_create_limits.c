...
/* Helpers */
static void dummy_task(void* arg){ (void)arg; for(;;){ hrt_yield(); } }
...
static void test_max_tasks_enforced(void){
    ...
    /* Create up to HEARTOS_MAX_TASKS tasks successfully */
    int created = 0;
    static uint32_t stacks[HEARTOS_MAX_TASKS][1024];
    ...
    T_ASSERT_EQ_INT(HEARTOS_MAX_TASKS, created, "should create up to HEARTOS_MAX_TASKS tasks");

    /* Next creation should fail */
    static uint32_t extra[1024];
    int fail_tid = hrt_create_task(dummy_task, NULL, extra, 1024, &a);
    T_ASSERT_TRUE(fail_tid < 0, "creating beyond HEARTOS_MAX_TASKS should fail");
}
...
static void coop_peer(void* arg){ (void)arg; for(;;){
        /* busy until we decide to sleep once */
        if (++g_peer_slept >= 50000){ g_peer_slept = 1; hrt_sleep(1); hrt__test_stop_scheduler(); hrt_yield(); }
    } }
static void rr_peer(void* arg){ (void)arg; for(;;){ if (!g_peer_slept) ++g_rr_before_peer_sleep; ++g_rr_iters; hrt_sleep(1);} }

static void test_attr_null_inherits_default_slice_zero(void){
    hrt__test_reset_scheduler_state(); g_rr_iters=0; g_rr_before_peer_sleep=0; g_peer_slept=0;
