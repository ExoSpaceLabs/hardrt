...
static void sleeper0(void* arg){ (void)arg; for(;;){
...
static void test_sleep_zero_is_one_tick_minimum(void){
    hrt__test_reset_scheduler_state(); g_s0_elapsed = -1;
...
const test_case_t* get_tests_sleep_zero(int* out_count){ if(out_count)*out_count=(int)(sizeof(CASES)/sizeof(CASES[0])); return CASES; }
