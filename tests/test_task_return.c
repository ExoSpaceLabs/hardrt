...
static void task_returns_immediately(void* arg){ (void)arg; /* just return */ }
static void worker_task(void* arg){ (void)arg; for(;;){ ++g_worker_iters; hrt_sleep(1); if (g_worker_iters >= 10){ hrt__test_stop_scheduler(); hrt_yield(); } } }

static void test_task_return_does_not_crash(void){
    hrt__test_reset_scheduler_state(); g_worker_iters = 0;
...
const test_case_t* get_tests_task_return(int* out_count){ if(out_count)*out_count=(int)(sizeof(CASES)/sizeof(CASES[0])); return CASES; }
