...
int main(void){
    ...
    int n = 0; const test_case_t* g = NULL;

    g = get_tests_identity(&n);     append_group(g, n, registry, &total);
    g = get_tests_sleep_stop(&n);   append_group(g, n, registry, &total);
    g = get_tests_rr_yield(&n);     append_group(g, n, registry, &total);
    g = get_tests_rr_sleep(&n);     append_group(g, n, registry, &total);
    g = get_tests_priority(&n);     append_group(g, n, registry, &total);
    g = get_tests_coop_vs_rr(&n);   append_group(g, n, registry, &total);
    g = get_tests_tick_rate(&n);    append_group(g, n, registry, &total);
    g = get_tests_create_limits(&n); append_group(g, n, registry, &total);
    g = get_tests_runtime_tuning(&n);append_group(g, n, registry, &total);
    g = get_tests_fifo_order(&n);   append_group(g, n, registry, &total);
