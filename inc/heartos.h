 ...
 */
    typedef enum { HRT_READY=0,  /**< Runnable (on ready queue) */
 ...
 */
    typedef struct {
        uint32_t       tick_hz;        /**< Tick frequency in Hz (default: 1000) */
        hrt_policy_t   policy;         /**< Scheduling policy (default: PRIORITY_RR) */
        uint16_t       default_slice;  /**< RR time-slice in ticks; 0 disables RR (cooperative) */
    } hrt_config_t;
 ...
 */
     ...
     * @note Called by hrt_init(). Port must call hrt__tick_isr() at each tick.
     */
