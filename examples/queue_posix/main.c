...
int main(void) {
    ...
    if (hrt_create_task(producer, NULL, stack_prod, sizeof(stack_prod) / sizeof(stack_prod[0]), &p0) < 0) {
        puts("create producer failed");
        return 1;
    }
    if (hrt_create_task(consumer, NULL, stack_cons, sizeof(stack_cons) / sizeof(stack_cons[0]), &p1) < 0) {
