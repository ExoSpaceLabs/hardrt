...
# build HardRT and hardrt_h755_blinky_cpp application
$(pwd)/scripts/build-lib-stm32h7xx.sh --app $(pwd)/examples/hardrt_h755_blinky_cpp --build-type Release
...
openocd -s /usr/share/openocd/scripts   -f scripts/openocd_h755_clean.cfg   -c "init; reset halt; \
      ...
      program examples/hardrt_h755_blinky_cpp/build-cortex_m/hardrt_h755_blinky_cpp.elf verify; \
      reset halt; shutdown"
...
# gdb-multiarch -q examples/hardrt_h755_blinky_cpp/build-cortex_m/hardrt_h755_blinky_cpp.elf -batch -x scripts/gdb/timing.dbg
